export const HTML_TS = [
  ...
  },
  {
    ...
    response: [
      'id — unique, used for a single element (#id in CSS). class — can be applied to multiple elements (.class in CSS).',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      'action — URL where data is sent. method — method of sending (GET or POST).',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      '<!DOCTYPE html> — indicates HTML5. <html> — the root element. <head> — technical information about the page (meta tags, styles, fonts, etc.). <body> — the visible content of the page.',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      'Define the form structure using the <form> tag. 🔐 Add fields (input, textarea, select). ✅ Add built-in HTML5 validation. 🧪 (Optional) add JavaScript for more flexible validation.',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      '1. Purpose and semantics <img> — content image. This is part of the page content that is important to the user (photo, icon, illustration). background-image — background image. Visual design that is not content, but complements the design. 2. Location in the DOM <img> — a separate element in the HTML structure, it can be influenced through attributes, it is subject to SEO and accessibility (alt-text). background-image — is set via CSS to any block (div, section, etc.), does not create additional HTML elements. 3. Accessibility <img> supports the alt attribute to describe the image, which is important for visually impaired users and SEO. background-image — has no alternative text and is not readable by screen readers. 4. Size and positioning <img> — the size can be controlled via width/height or CSS attributes, the image is rendered as a separate element. background-image — you can control the position, size (via background-position, background-size), repetition (background-repeat). 5. Impact on the flow of the document <img> takes up space in the document (inline element). background-image — does not take up space, the image is just the background behind which the content is located.',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      'SVG icons (separate files or inline), SVG sprites (collection of icons in one file), Icon fonts — FontAwesome, IcoMoon, LineAwesome, PNG or WebP images, CSS sprites',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      '🟢 Pros: All icons in one file → fewer HTTP requests Good caching, Easy to use via <use>. 🔴 Cons: Requires pre-compilation (via SVG sprite generator). Connection is a bit more complicated.',
    ],
    fullResponse: '',
  },
  {
    question: 'Icons via font (icon fonts) — FontAwesome, IconMoon, Font Awesome pluses and minuses',
    response: [
      '🟢 Pros: Easy to connect via class (<i class="fa fa-search">), Styled as a font (via font-size, color). 🔴 Cons: Outdated approach, Not as sharp as SVG at high pixel density, Depends on third-party library',
    ],
    fullResponse: '',
  },
  {
    ...
    response: [
      "🟢 Pros: Simplicity: works everywhere, No SVG support required. 🔴 Cons: No lossless scaling, Can't change color via CSS, Larger files compared to SVG",
    ],
    fullResponse: '',
  },
  {
    ...
    response: [
      '🟢 Pros: One request - less load on the server, Good for many small icons. 🔴 Cons: Complex positioning, Not flexible in styling, Does not support :hover, currentColor and responsiveness',
    ],
    fullResponse: '',
  },
  {
  ...
  },
  {
    ...
    response: [
      'This is a set of native browser technologies for creating custom HTML elements with their own logic and styles. Consists of: Custom Elements – creating new HTML tags (<my-button>), Shadow DOM – isolating markup and styles, HTML Templates – templates that are not rendered immediately. Slots – passing content to templates',
    ],
    fullResponse: '',
  },
  {
    ...
    response: [
      "Shadow DOM is a way to encapsulate the DOM tree and styles inside a component. ✅ Benefits: Styles are isolated from external CSS HTML structure is hidden from the main DOM. Avoids class, ID and style conflicts. Can use mode: 'open' (accessible via JS) or 'closed' (not accessible)",
    ],
    fullResponse: '',
  },
] as const;